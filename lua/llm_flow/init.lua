...
local function ensure_install()
  ...
  if not utils.check_directory_exists(utils.llm_flow_path) then
    if not utils.git_clone('festeh/llm_flow', utils.llm_flow_path) then
  ...
  if not utils.check_executable_exists() then
    -- Ensure bin directory exists
    vim.fn.mkdir(utils.llm_flow_path .. '/bin', 'p')

    if not utils.build_go_executable() then
